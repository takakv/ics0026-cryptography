...
def main():
    nums = [secrets.randbits(3072), secrets.randbits(3072),
            secrets.randbits(3072)]
    # Sort the numbers such that a < b < c.
    nums.sort()
    ...
    # gmpy2.powmod_sec() requires an odd modulus.
    if gmpy2.is_even(mod):
        mod += 1
